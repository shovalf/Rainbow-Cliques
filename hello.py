...
def run_dm(graph, labels, sg_sz):
    ...
    final = find_max_vertices(labels_clique, labels, list_v, num=5)
...
def first_sol(g, node2label, label2node, t):
    total_time_, c_n_hat, final_clique_, avg_ = run_dm(g, node2label, 200)
...
a=0
t_ = time.time()
graph_, node_to_label, label_to_node, avg_len_label = create_graph(9)
my = time.time() - t_
print(my)
t_ = time.time()
if a==1:
